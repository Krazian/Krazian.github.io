...
let dealCards = () => {
	...
	splitHand =[];
	dealerHand = [];
	playerHand.push(getACard(deck));
	dealerHand.push(getACard(deck));
	playerHand.push(getACard(deck));
	dealerHand.push(getACard(deck));
};

//Randomly selects suit and value then removes from master deck
let getACard = (deckOfCards) => {
	let randomSuit = () => Math.floor(Math.random() * deckOfCards.length);
	let suit = randomSuit();
	let randomCard = () => Math.floor(Math.random() * deckOfCards[suit].length);
	let card = randomCard();
	let fullCard = deckOfCards[suit][card];
	deckOfCards[suit].splice(card,1); // removes card from the deck
	return fullCard
};
...
let moneyManage = () => {
    ...
    location.reload(true);
		...
		let pic = $("<img>").attr("src","http://cdn2.hubspot.net/hub/378747/file-2184119855-png/broke-monopoly-guy.png");
...
let isNum = () => {
	...
	for (let x = 0; x < ($(".bet").val().length); x++){
